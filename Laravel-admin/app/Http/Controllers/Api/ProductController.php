...
class ProductController extends Controller
{
    ...
    public function getRecommendations(Request $request)
    {
        ...
        if ($userId) {
            ...
            $searchTerms = SearchHistory::where('user_id', $userId)
                ->orderBy('created_at', 'desc')
                ->limit(5)
                ->pluck('query');
        }
        ...
        $categories = Product::whereIn('id', $recentlyViewed)->pluck('id_category')->unique();

        $recommendedQuery = Product::with([
            'productVariants',
            'brand',
            'sub_category',
            'discount'
        ...
        if ($categories->isNotEmpty() || $searchTerms->isNotEmpty()) {
            $recommendedQuery->where(function ($q) use ($categories, $searchTerms) {
                if ($categories->isNotEmpty()) {
                    $q->orWhereIn('id_category', $categories);
                }
            ...
            // Nếu không có lịch sử, chỉ lấy sản phẩm HOT
            $recommendedQuery->where('hot_product', 1);
        }

        // Bao gồm cả sản phẩm đã xem để người dùng dễ tìm lại
        $recommendedProducts = $recommendedQuery->limit(8)->get();

        // Fallback if not enough recommendations
        if ($recommendedProducts->count() < 4) {
            $fallback = Product::with(['productVariants', 'brand', 'sub_category', 'discount'])
                ->where('status', '>', 0)
                ->where('hot_product', 1)
                ->limit(8)
                ->get();
            $recommendedProducts = $recommendedProducts->concat($fallback)->unique('id')->take(8);
        }
